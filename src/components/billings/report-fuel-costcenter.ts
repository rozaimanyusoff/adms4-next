...
export async function generateFuelCostCenterReport({ stmt_id }: { stmt_id: number }) {
    ...
    const addHeaderFooter = async (pageNumber: number, totalPages: number) => {
        ...
        doc.setFontSize(8);
        const pageHeight = doc.internal.pageSize.getHeight();
        doc.text('This document is generated by ADMS4', pageWidth - 80, pageHeight - 35, { align: 'right' });
        
        // Add page number
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(8);
        doc.text(`Page ${pageNumber} of ${totalPages}`, pageWidth - 95, pageHeight - 5, { align: 'right' });
    };
