...
export async function generateFuelCostCenterReport({ stmt_id }: { stmt_id: number }) {
    ...
    // Add header and footer to first page (will be updated later with correct total pages)
    ...
    const stmtLabel = determineStatementLabel(data);
    doc.text(`FUEL BILLS - ${stmtLabel}`, 15, 64);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(9);
